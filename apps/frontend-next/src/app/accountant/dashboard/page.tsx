...
export default function AccountantDashboard() {
  ...
  const load = async (background = false) => {
    try {
      ...
      try {
        ...
        const paidMap: any = { all: '', paid: 'true', unpaid: 'false', today: 'today', week: 'week', month: 'month' }
        if (paidMap[studentFilter]) q.set('paid', paidMap[studentFilter])
        const rl = await fetch(`/api/local/profiles/students/with-fees?${q.toString()}`)
        const sj = await rl.json(); setStudents(sj.items || [])
      } catch { setStudents([]) }
  ...
  React.useEffect(() => { (async()=>{
    // refresh only students on filter change
    try {
      const q = new URLSearchParams()
      const paidMap: any = { all: '', paid: 'true', unpaid: 'false', today: 'today', week: 'week', month: 'month' }
      if (paidMap[studentFilter]) q.set('paid', paidMap[studentFilter])
      ...
      <div className="card" style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        {loading ? <p className="subtitle">Loading…</p> : tab==='fees' ? (
          ...
          <div>
            ...
            <table className="table">
              <thead><tr><th align="left">Created</th><th align="left">Title</th><th align="left">Target</th><th align="left">Purpose</th><th align="right">Total</th><th>Actions</th></tr></thead>
              <tbody>
                {adhoc.map((a)=> {
                  const t = a.target
                  let tText = '-'
                  ...
                  return (
                    <tr key={a.id}><td>{new Date(a.createdAt).toLocaleString()}</td><td>{a.title}</td><td>{tText}</td><td>{a.purpose||'-'}</td><td align="right">{Number(a.total||0).toLocaleString('en-IN',{style:'currency',currency:'INR'})}</td><td style={{ textAlign:'right', display:'flex', gap:8, justifyContent:'flex-end' }}><button className="btn-ghost" onClick={async()=>{ if(!confirm('Send this ad-hoc fee to recipients now?')) return; await fetch(`/api/local/accounting/adhoc/${a.id}/send`,{ method:'POST'}); await load(true) }}>Send</button><button className="btn-ghost" onClick={async()=>{ if(!confirm('Delete ad-hoc fee?')) return; await fetch(`/api/local/accounting/adhoc/${a.id}/delete`,{ method:'POST'}); await load() }}>Delete</button></td></tr>
                  )
                })}
              </tbody>
            </table>
                  ...
                  <React.Fragment key={i}>
                    <tr style={{ borderTop: '1px solid var(--panel-border)' }}>
                      ...
                      <td style={{ textAlign:'right' }}>
                        {s.appId ? <a className="btn-ghost" href={`/accountant/fees/${encodeURIComponent(s.appId)}`}>Edit Fees</a> : null}
                      </td>
                    </tr>
