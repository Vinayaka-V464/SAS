...
import { getClasses, getSectionsForClass, getSubjects, rosterBy, readMarks, saveMarks, listTestsBySubject, getAssignedClassesForTeacher, getAssignedSectionsForTeacher, getAssignedSubjectsForTeacher, getClassSubjects } from '../data'
...
export default function TeacherMarks() {
  ...
  return (
    <div className="dash">
      ...
      <p className="subtitle">Enter subject-wise marks and save by test name.</p>
      ...
      <div style={{display:'grid', gap:12, marginTop:12}}>
        <div className="row">
          ...
          <select className="input select" value={subject} onChange={e=>setSubject(e.target.value)}>
            {(allowedSubjects.length ? allowedSubjects : getSubjects()).map(s=> <option key={s}>{s}</option>)}
          </select>
        </div>
        <div className="row">
