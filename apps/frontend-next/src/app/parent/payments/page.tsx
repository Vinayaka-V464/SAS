...
export default function ParentPaymentsPage() {
  ...
  const pay = async (idx: number) => {
    if (!appId) return
    try {
      await fetch('/api/local/parent/fees/pay', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ appId, index: idx }) })
      await load(true)
      setMessage('Payment recorded')
      setTimeout(() => setMessage(''), 2500)
    } catch { setMessage('Payment failed'); setTimeout(() => setMessage(''), 2500) }
  }

  const payAdhoc = async (id: string) => {
    try { await fetch('/api/local/parent/fees/adhoc/pay', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ billId: id }) }); await load(true); setMessage('Ad-hoc payment recorded'); setTimeout(()=>setMessage(''), 2000) } catch { setMessage('Payment failed'); setTimeout(()=>setMessage(''), 2000) }
  }
